...
def extract_tau_W_values(experiment_config):
    """Extract tau_W values from experiment configuration."""
    param_grid = experiment_config.get("param_grid", {})
    tau_W_values = param_grid.get("tau_W", [])
    print(f"Extracted tau_W values: {tau_W_values}")
    return sorted(tau_W_values)


def plot_time_scale_ratio():
    """Create time scale ratio plot with 2x2 grid of panels."""

    # Load style and data
    style = load_style()
    data_dict, metadata, time_values = load_experiment_data("time_scale_ratio")

    # Extract tau_W values from experiment config
    experiment_config = metadata["experiment_config"]
    tau_W_values = extract_tau_W_values(experiment_config)

    if not tau_W_values:
        raise ValueError("No tau_W values found in experiment configuration")
    ...
    for i, (ax, label) in enumerate(zip(panels, panel_labels)):
        ...
        if i >= 2:  # Bottom panels (C, D)
            ax.set_xlabel("$\\tau_W$", fontsize=axis_size)
            ax.set_xticks(x_positions)  # Use log positions
            ax.set_xticklabels(
                [str(tau) for tau in tau_W_values]
    ...
    output_path = figures_dir / "time_scale_ratio.pdf"
